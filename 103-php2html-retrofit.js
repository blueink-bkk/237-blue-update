...
function fix_base_url(fn,base_url) {
  console.log(`fix_base_url <${fn}>`)
  return fs.readFileSync(fn,'utf8')
  .replace(/<\?= base_url; \?>/g,base_url) // must be first
  .replace(/<\?php define.*\?>/s,'')
}
...
//  .replace(/<\?php.*\?>/s,'')
  ...
  ***********************/
  ...
  const head = fix_base_url(path.join(input_folder,lang,'head.php'),'/');
  const header = fix_base_url(path.join(input_folder,lang,'header.php'),'/');
  const footer = fix_base_url(path.join(input_folder,lang,'footer.php'),'/');
  const sidebar = fix_base_url(path.join(input_folder,lang,'sidebar.php'),'/');
...
//      .replace(/^[^]*<!DOCTYPE/mi,'<!DOCTYPE')
      const html = fix_base_url(file.path,'/')
